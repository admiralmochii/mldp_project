...
def user_input_features():
    ...
    date = st.slider('Date of sale', max_value=datetime(2017,12,31),min_value=datetime(2016,1,1), format="YYYYMM")
    distance = st.slider('Distance from Melbourne CBD', 0.0, 9.75, 29.8)
    bathrooms = st.slider('No. of Bathrooms', 0.0, 1.0, 4.0, step=1.0)
    cars = st.slider('No. of carspots', 0.0, 2.0, 10.0, step=1.0)
    land = st.slider('Land Size', 0.0, 451.0, 4497.0)
    prop_count = st.slider('No. of property in the area', 389.0, 7416.0, 21650.0)
    house_type = st.selectbox("Type of house", options=(
        "Bedroom(s)",
